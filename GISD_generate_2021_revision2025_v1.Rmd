...
library(kableExtra) # Erweiterte Markdown-Tabellen
...
```{r paths internal, include=FALSE}
...
table_regionen <- tibble(
  Ebene = c("Gemeinden",
            ...
            "NUTS-2"),
  Anzahl = c(length(unique(id_for_table$Gemeindekennziffer)),
...
Quelle der Indikatoren sind INKAR-Daten im Excel-Format, die zu jeder Revision aus der INKAR-Datenbank heruntergeladen werden. Tabelle \@ref(tab:tabelle_indikatoren) stellt die Indikatoren und deren respektive Ebene dar. Für einige Indikatoren, die in absoluten Zahlen vorliegen, werden in diesem Kapitel Quoten bzw. Anteile berechnet: Arbeitslose pro 1000 Einwohner im Erwerbsalter, Anteil der Schulabgänger ohne Abschluss sowie der Anteil der Schulabgänger mit akad. Abschluss an allen SV-Beschäftigten. Zusätzlich werden Gemeinden ohne Bevölkerung ausgeschlossen.
...
# Es folgt eine Schleife mit dem Ziel, einen Datensatz zu bauen, wo jedes Datenjahr von jeder Gemeinde, jedem Gemeindeverband,
...
# Es wird stattdessen provisorisch ein Datensatz ("IRIS") herangezogen, der uns auf Anfrage von INKAR zugeschickt wurde.
...
# Notiz: Momentan der einzige df auf Gemeindeverbandsebene, nach INKAR-Release neue Lösung benötigt
suppressMessages(
...
```{r maintenance gemeindenohnebevoelkerung, include=FALSE}
...
```{r tabelle_indikatoren, echo=FALSE}
...
kbl(table_levels,
  ...
  kable_styling(fixed_thead = TRUE) # Spaltennamen schweben beim Scrollen mit (weil die Tabelle ist lang)
...
```{r plot-g8, figures-side, fig.show="hold", out.width="50%", echo=FALSE}
...
```{r plot-ow, figures-side, fig.show="hold", out.width="50%", echo=FALSE}
...
```{r plot-messaenderung, figures-side, fig.show="hold", out.width="50%", echo=FALSE}
...
Fehlende Werte auf einem Indikator werden anhand einer Regression imputiert, die vom gemeindespezifischen Zeitreihenmittelwert desselben Indikators informiert wird. Früher wurde anhand der Arbeitslosigkeit und der Schulabgänger mit Hochschulreife imputiert, aber das brachte einen mit diesen Variablen korrelierenden Bias mit sich. Daher wird die Imputation nun nur noch vom Indikator selbst informiert. Die Verteilung der Missings vor der Imputation ist in Tabelle \@ref(tab:tabelle_missings) ersichtlich.
...
#   left_join(Workfile_imputed %>% pivot_longer(cols = 7:21,
#                                               names_to = "Indikator",
#                                               values_to = "value") %>%
#               # Missings aufsummieren
#               group_by(Indikator) %>%
...
# Nur noch relevante Variablen mitnehmen
Workfile_imputed <- Workfile_imputed %>% 
  select(Gemeindekennziffer,
         Jahr,
         GVBKennziffer,
         Kreiskennziffer,
...
table_miss <- Workfile %>%
  ...
  mutate(Indikator = !!c("Arbeitslosigkeit", "Beschäftigtenquote", "Bruttoverdienst (log.)",
                       "Einkommensteuer (log.)", "Haushaltseinkommen (log.)", "Schuldnerquote",
                       "Beschäftigte mit akad. Abschluss (adj.)", "Beschäftigte ohne Abschluss (adj.)", "Schulabgänger ohne Abschluss (adj.)"))

kbl(table_miss,
    caption = "Anzahl fehlende Werte pro Indikator (jahresweise)")

rm(missings, table_miss)
...
```{r plot-indicators, fig.height=11.25, fig.width=20, echo=FALSE, eval=FALSE}
...
```{r pca-teilscores, figures-side, fig.show="hold", out.width="33%", fig.height=10}
...
```{r tabelle_ladungen, echo=FALSE}

# Dimension aus Tabelle entfernen (wird gleich händisch ergänzt)
GISD_Components <- GISD_Components %>% 
  select(-Dimension)
...
kbl(GISD_Components, caption = "Übersicht über die Komponente des GISD") %>%
  kable_styling() %>% 
  footnote(
    ...
    general = paste0(
      ...
      ": ηBeschäftigung = ",eigenvalues_a[1],
      "; ηEinkommen = ",eigenvalues_e[1],
      "; ηBildung = ",eigenvalues_b[1],
      ...
      ": rBeschäftigung|Einkommen = ",cor_table["a","e"],
      "; rBeschäftigung|Bildung = ",cor_table["a","b"],
      "; rEinkommen|Bildung = ",cor_table["e","b"],
...
# # Plots ausgeben lassen (REMOVE COMMENT BEFORE KNITTING)
# plot_scores
# plot_scores_imp1
# plot_scores_imp2
plot_ae
...
```{r maintenance faktoren adjusted, include=FALSE}
...
for(current_ebene_id in exportlist$Kennziffern) {
  ...
  # Beachte: Bevölkerungszahlen im Datensatz sind nicht jahresaktuell,
