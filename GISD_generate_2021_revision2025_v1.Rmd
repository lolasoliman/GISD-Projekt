...
# und jedem Kreis eine eigene Zeile bildet. Es wird jede Zeile entsprechend ihrer Gebietsebene und Jahr mit den passenden Indikatoren
...
level_table <- data.frame(Indikator=names(Basedata)[3:length(Basedata)],
                          Tiefe_Indikator=c("Gemeinde", # Arbeitslosigkeit
                                            "Gemeinde", # Beschaeftigtenquote
                                            "Kreis",    # Bruttoverdienst
                                            "Gemeinde", # Erwerbsfaehige Bevölkerung
                                            "Kreis",    # Beschaeftigte mit akad. Abschluss
                                            "Kreis",    # Beschaeftigte ohne Abschluss
                                            "Kreis",    # Schulabgänger ohne Abschluss
                                            "Gemeindeverband", # Einkommensteuer - Switch auf Gemeinde sobald INKAR veröffentlicht
                                            "Kreis",    # Haushaltseinkommen
                                            "Kreis"))   # Schuldnerquote
...
knitr::kable(missings, col.names = c("Indikator",
                                     "Missings vor Imputation",
                                     "Missings nach Imputation"),
             caption = "Übersicht über Imputation fehlender Werte")
...
```{r pca-teilscores, figures-side, fig.show="hold", out.width="33%", out.height="200%"}
...
Components_A <- data.frame(Indikator=rownames(TS_Arbeitswelt.pca$rotation),
...
Components_E <- data.frame(Indikator=rownames(TS_Einkommen.pca$rotation),
                           Dimension="Einkommen",
                           Anteil=unname(TS_Einkommen.pca$rotation^2),
                           Score=unname(TS_Einkommen.pca$rotation * TS_Einkommen.pca$sdev[1]))

Components_B <- data.frame(Indikator=rownames(TS_Bildung.pca$rotation),
                           Dimension="Bildung",
...
knitr::kable(GISD_Components, caption = "Übersicht über die Komponente des GISD") %>%
  footnote(general = paste0(
  "Darstellung der Faktorladungen auf der ersten Hauptkomponente für die einzelnen Dimensionen. Eigenwerte der ersten Komponenten: ηBeschäftigung = ",eigenvalues_a[1],"; ηEinkommen = ",eigenvalues_e[1],"; ηBildung = ",eigenvalues_b[1],". Die Eigenwerte der zweiten und dritten Komponenten liegen jeweils unter 0,85. Korrelationen zwischen den Scores der Teildimensionen für das Jahr ",latestyear,": rBeschäftigung|Einkommen = ",cor_table["a","e"],"; rBeschäftigung|Bildung = ",cor_table["a","b"],"; rEinkommen|Bildung = ",cor_table["e","b"],".
